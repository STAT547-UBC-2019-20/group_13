...
histplot <- function(xaxis="mpg", num_bins=3) { 
  ...
  # Make the boxplot object, assign it to 'box'
  box <- suiciderates %>%
    filter(country == 'Canada', 'United states', 'Mexico') %>%
           ggplot(data = df, aes(x=!!sym(xaxis), y = suicides_no, fill = sex)) +
    geom_boxplot() +
    labs(y = "Number of Suicides", x = xaxis) +
      theme_minimal(20) +
